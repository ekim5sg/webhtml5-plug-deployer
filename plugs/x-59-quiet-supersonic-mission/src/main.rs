...
fn play_audio(audio_ref: &NodeRef, looped: bool, volume: f64, reset_to_start: bool) {
...
fn pause_audio(audio_ref: &NodeRef, reset_to_start: bool) {
    if let Some(audio) = audio_ref.cast::<HtmlAudioElement>() {
        let _ = audio.pause();
        if reset_to_start {
            audio.set_current_time(0.0);
...
fn step_game(game: &mut GameState) {
    ...
    let target_speed =
        (110.0 + game.throttle * 5.2 - gear_penalty - pitch_penalty).clamp(80.0, 620.0);

    game.speed += (target_speed - game.speed) * 0.065;

    let lift = ((game.speed - 150.0) / 330.0).clamp(0.0, 1.0);
    let base_sink = if game.altitude > 0.0 {
        6.0 - lift * 4.0
    } else {
        0.0
    };
    ...
    match game.card {
        ...
        3 => {
            if (435.0..=485.0).contains(&game.speed)
                && (18500.0..=21500.0).contains(&game.altitude)
            {
                game.stable_ticks += 1;
        ...
        4 => {
            if (430.0..=480.0).contains(&game.speed)
                && (18000.0..=22000.0).contains(&game.altitude)
            {
                game.stable_ticks += 1;
...
fn app() -> Html {
    ...
    let engine_audio_ref = use_node_ref();
    let victory_audio_ref = use_node_ref();
    let prev_running = use_mut_ref(|| false);
    let prev_mission_done = use_mut_ref(|| false);
    let prev_crashed = use_mut_ref(|| false);
...

    {
        ...
        let mission_done = game.mission_done;
        let crashed = game.crashed;
        let engine_audio_ref = engine_audio_ref.clone();
        let victory_audio_ref = victory_audio_ref.clone();
        let prev_running = prev_running.clone();
        let prev_mission_done = prev_mission_done.clone();
        let prev_crashed = prev_crashed.clone();

        use_effect_with((running, mission_done, crashed), move |(running, mission_done, crashed)| {
            let was_running = *prev_running.borrow();
            let was_mission_done = *prev_mission_done.borrow();
            let was_crashed = *prev_crashed.borrow();

            if *running && !was_running {
                pause_audio(&victory_audio_ref, true);
                play_audio(&engine_audio_ref, true, 0.45, true);
            }

            if *mission_done && !was_mission_done {
                pause_audio(&engine_audio_ref, true);
                play_audio(&victory_audio_ref, false, 0.9, true);
            }

            if *crashed && !was_crashed {
                pause_audio(&engine_audio_ref, true);
            }

            if !*running && !*mission_done && !*crashed && was_running {
                pause_audio(&engine_audio_ref, true);
            }

            *prev_running.borrow_mut() = *running;
            *prev_mission_done.borrow_mut() = *mission_done;
            *prev_crashed.borrow_mut() = *crashed;
