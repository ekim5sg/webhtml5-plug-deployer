...
// External WAV files served from the Hostek web root under assets/audio/
...
#[derive(Clone, Copy, PartialEq, Eq)]
enum MobilePanel { Apollo, Center, Orion }
...
// ---------------------------------------------------------------------------
// Audio — WAV playback via HtmlAudioElement
// ---------------------------------------------------------------------------

/// Play an external WAV file by URL path.  The browser handles decoding and
/// playback; no AudioContext or oscillator needed.
fn play_wav_cue(src: &str) {
...
fn km_to_miles(km: f64) -> f64 { km * 0.621_371 }
fn kps_to_mph(kps: f64) -> f64 { kps * 2236.936 }
...
fn build_mission_state(t: f64) -> MissionState {
    ...
    let (altitude_km, velocity_kps, downrange_km,
         distance_from_earth_km, distance_to_moon_km,
         fuel_pct, power_pct, cabin_temp_c,
         pitch_deg, yaw_deg, roll_deg, comm_link_pct) = match phase {
        ...
        MissionPhase::Launch => {
            ...
            (alt, vel, dr, earth, moon,
             lerp(100.0, 84.0, p) + tiny_noise(0.2),
             lerp(100.0, 99.0, p) + tiny_noise(0.05),
             lerp(22.0, 24.0, p)  + tiny_noise(0.15),
             lerp(90.0, 10.0, p)  + tiny_noise(0.6),
             lerp(0.0, 1.2, p)    + tiny_noise(0.2),
             lerp(0.0, 3.5, p)    + tiny_noise(0.3),
             lerp(94.0, 98.0, p)  + tiny_noise(0.2))
        }
        ...
        MissionPhase::OrbitCheckout => {
            ...
            let moon  = 384400.0 - earth;
            (alt, vel, dr, earth, moon,
             lerp(84.0, 82.0, p) + tiny_noise(0.15),
             lerp(99.0, 97.5, p) + tiny_noise(0.08),
             lerp(24.0, 23.0, p) + tiny_noise(0.1),
             lerp(10.0, 0.0, p)  + tiny_noise(0.3),
             lerp(1.2, 0.2, p)   + tiny_noise(0.15),
             lerp(3.5, 0.4, p)   + tiny_noise(0.2),
             lerp(98.0, 99.0, p) + tiny_noise(0.15))
        }
        ...
        MissionPhase::Tli => {
            ...
            let moon  = 384400.0 - earth;
            (alt, vel, dr, earth, moon,
             lerp(82.0, 65.0, p)  + tiny_noise(0.2),
             lerp(97.5, 96.0, p)  + tiny_noise(0.08),
             lerp(23.0, 24.5, p)  + tiny_noise(0.12),
             lerp(0.0, -4.0, p)   + tiny_noise(0.35),
             lerp(0.2, 0.0, p)    + tiny_noise(0.12),
             lerp(0.4, 1.0, p)    + tiny_noise(0.18),
             lerp(99.0, 97.0, p)  + tiny_noise(0.18))
        }
        ...
        MissionPhase::CoastOut => {
            ...
            (alt, vel, dr, earth, moon,
             lerp(65.0, 61.0, p) + tiny_noise(0.12),
             lerp(96.0, 92.0, p) + tiny_noise(0.09),
             lerp(24.5, 22.8, p) + tiny_noise(0.12),
             lerp(-4.0, 0.0, p)  + tiny_noise(0.2),
             lerp(0.0, 0.4, p)   + tiny_noise(0.1),
             lerp(1.0, 359.0, p) + tiny_noise(0.35),
             lerp(97.0, 93.0, p) + tiny_noise(0.25))
        }
        ...
        MissionPhase::LunarFlyby => {
            ...
            let moon  = clamp(384400.0 - earth, 0.0, 384400.0);
            (alt, vel, dr, earth, moon,
             lerp(61.0, 60.0, p) + tiny_noise(0.08),
             lerp(92.0, 91.0, p) + tiny_noise(0.08),
             lerp(22.8, 23.4, p) + tiny_noise(0.1),
             lerp(0.0, 14.0, p)  + tiny_noise(0.2),
             lerp(0.4, -0.5, p)  + tiny_noise(0.1),
             lerp(359.0, 2.0, p) + tiny_noise(0.3),
             lerp(93.0, 90.0, p) + tiny_noise(0.3))
        }
        ...
        MissionPhase::CoastHome => {
            ...
            let moon  = clamp(384400.0 - earth, 0.0, 384400.0);
            (alt, vel, dr, earth, moon,
             lerp(60.0, 56.0, p) + tiny_noise(0.12),
             lerp(91.0, 87.0, p) + tiny_noise(0.09),
             lerp(23.4, 22.2, p) + tiny_noise(0.1),
             lerp(14.0, -2.0, p) + tiny_noise(0.2),
             lerp(-0.5, 0.2, p)  + tiny_noise(0.1),
             lerp(2.0, 356.0, p) + tiny_noise(0.35),
             lerp(90.0, 95.0, p) + tiny_noise(0.24))
        }
        ...
        MissionPhase::ReturnBurn => {
            ...
            let moon  = clamp(384400.0 - earth, 0.0, 384400.0);
            (alt, vel, dr, earth, moon,
             lerp(56.0, 52.0, p)  + tiny_noise(0.1),
             lerp(87.0, 85.0, p)  + tiny_noise(0.07),
             lerp(22.2, 23.2, p)  + tiny_noise(0.1),
             lerp(-2.0, -12.0, p) + tiny_noise(0.2),
             lerp(0.2, 0.0, p)    + tiny_noise(0.08),
             lerp(356.0, 1.0, p)  + tiny_noise(0.28),
             lerp(95.0, 96.0, p)  + tiny_noise(0.18))
        }
        ...
        MissionPhase::Reentry => {
            ...
            (alt, vel, dr, earth, moon,
             lerp(52.0, 50.0, p) + tiny_noise(0.05),
             lerp(85.0, 83.0, p) + tiny_noise(0.06),
             lerp(23.2, 27.0, p) + tiny_noise(0.2),
             lerp(-12.0, 84.0, p)+ tiny_noise(0.8),
             lerp(0.0, 0.5, p)   + tiny_noise(0.15),
             lerp(1.0, 0.0, p)   + tiny_noise(0.22),
             comm)
        }
    };
...
fn orion_comm_label(phase: MissionPhase, pct: f64) -> &'static str {
    if phase == MissionPhase::Reentry && pct < 15.0 { "Telemetry Blackout" }
...
fn to_orion_display(state: &MissionState) -> OrionDisplay {
    OrionDisplay {
        ...
        attitude:       format!("P {:.1}°  Y {:.1}°  R {:.1}°", state.pitch_deg, state.yaw_deg, state.roll_deg),
        ...
        nav_chip: if reentry_blackout_active(state) { "Plasma Blackout" } else { parachute_stage(state) },
        ...
        <div class="chart-wrap">
            ...
            <div class="chart-frame">
                <svg viewBox="0 0 320 110" aria-label={props.label}>
                    <line x1="8" y1="102" x2="312" y2="102"
                          stroke="rgba(255,255,255,0.12)" stroke-width="1" />
                    <path d={d} fill="none" stroke={props.stroke_class}
...
fn app() -> Html {
    ...
    let audio_armed   = use_state(|| false);   // true once user clicks anything
    let audio_status  = use_state(|| "No cue fired yet".to_string());

    let time_ref          = use_mut_ref(|| 0.0_f64);
    let blackout_played   = use_mut_ref(|| false);
    let drogue_played     = use_mut_ref(|| false);
    let main_played       = use_mut_ref(|| false);
    let splash_played     = use_mut_ref(|| false);

    // Keep time_ref in sync with time_s so the Interval closure has lock-free access.
    {
    ...
    // Reentry audio cue effect
    // Fires WAV files at the appropriate phase_progress thresholds.
    // Requires audio_armed (at least one user gesture has occurred).
    {
        let audio_armed       = audio_armed.clone();
        let audio_status_h    = audio_status.clone();
        let blackout_played   = blackout_played.clone();
        let drogue_played     = drogue_played.clone();
        let main_played       = main_played.clone();
        let splash_played     = splash_played.clone();
        let state_for_cue     = build_mission_state(*time_s);

        use_effect_with(
            (state_for_cue.phase, (state_for_cue.phase_progress * 1000.0) as i64, *audio_armed),
            move |(_, _, armed)| {
                if *armed && state_for_cue.phase == MissionPhase::Reentry {
                    let p = state_for_cue.phase_progress;

                    if p >= 0.30 && !*blackout_played.borrow() {
                        play_wav_cue(AUDIO_BLACKOUT_WAV);
                        *blackout_played.borrow_mut() = true;
                        audio_status_h.set("Cue Fired: Blackout".to_string());
                    }
                    if p >= 0.72 && !*drogue_played.borrow() {
                        play_wav_cue(AUDIO_DROGUE_WAV);
                        *drogue_played.borrow_mut() = true;
                        audio_status_h.set("Cue Fired: Drogue".to_string());
                    }
                    if p >= 0.84 && !*main_played.borrow() {
                        play_wav_cue(AUDIO_MAIN_WAV);
                        *main_played.borrow_mut() = true;
                        audio_status_h.set("Cue Fired: Main Chutes".to_string());
                    }
                    if p >= 0.98 && !*splash_played.borrow() {
                        play_wav_cue(AUDIO_SPLASH_WAV);
                        *splash_played.borrow_mut() = true;
                        audio_status_h.set("Cue Fired: Splashdown".to_string());
                    }
                }
                || ()
            },
    ...
    // Log + history update on every tick
    {
        let logs     = logs.clone();
    ...
    let phase_idx = MissionPhase::all().iter().position(|p| *p == state.phase).unwrap_or(0);
    ...
    let on_toggle_play = {
        let playing     = playing.clone();
        let audio_armed = audio_armed.clone();
        Callback::from(move |_| { audio_armed.set(true); playing.set(!*playing); })
    };
    ...
    let on_reset = {
        ...
        let time_ref       = time_ref.clone();
        let audio_armed    = audio_armed.clone();
        let audio_status_r = audio_status.clone();
        let blackout_played= blackout_played.clone();
        let drogue_played  = drogue_played.clone();
        let main_played    = main_played.clone();
        let splash_played  = splash_played.clone();
        ...
        Callback::from(move |_| {
            ...
            audio_status_r.set("No cue fired yet".to_string());
            playing.set(true);
            *time_ref.borrow_mut()       = 0.0;
            *blackout_played.borrow_mut()= false;
            *drogue_played.borrow_mut()  = false;
            *main_played.borrow_mut()    = false;
            *splash_played.borrow_mut()  = false;
            time_s.set(0.0);
    ...
    let on_speed_1 = { let s = speed.clone(); let a = audio_armed.clone();
        Callback::from(move |_| { a.set(true); s.set(1.0); }) };
    let on_speed_5 = { let s = speed.clone(); let a = audio_armed.clone();
        Callback::from(move |_| { a.set(true); s.set(5.0); }) };
    let on_speed_15 = { let s = speed.clone(); let a = audio_armed.clone();
        Callback::from(move |_| { a.set(true); s.set(15.0); }) };
    ...
    let on_jump = {
        ...
        let time_ref       = time_ref.clone();
        let audio_armed    = audio_armed.clone();
        let audio_status_j = audio_status.clone();
        let blackout_played= blackout_played.clone();
        let drogue_played  = drogue_played.clone();
        let main_played    = main_played.clone();
        let splash_played  = splash_played.clone();
        ...
        Callback::from(move |idx: usize| {
            ...
            audio_armed.set(true);
            audio_status_j.set("No cue fired yet".to_string());
            playing.set(false);
            *time_ref.borrow_mut()       = t;
            *blackout_played.borrow_mut()= false;
            *drogue_played.borrow_mut()  = false;
            ...
            *splash_played.borrow_mut()  = false;
            ...
            next.push(LogEntry { met: format_met(t), text: format!("Jumped to phase: {}", phase.label()) });
            logs.set(next);
        })
    };

    // Test Audio — verifies that the WAV file path is reachable and that the
    // browser will allow playback after a user gesture.
    let on_test_audio = {
        let audio_armed    = audio_armed.clone();
        ...
        Callback::from(move |_| {
            audio_armed.set(true);
            play_wav_cue(AUDIO_SPLASH_WAV);
            audio_status_t.set("Test: Splash WAV sent to browser".to_string());
        })
    };

    let show_apollo = { let mp = mobile_panel.clone(); let a = audio_armed.clone();
        Callback::from(move |_| { a.set(true); mp.set(MobilePanel::Apollo); }) };
    let show_center = { let mp = mobile_panel.clone(); let a = audio_armed.clone();
        Callback::from(move |_| { a.set(true); mp.set(MobilePanel::Center); }) };
    let show_orion  = { let mp = mobile_panel.clone(); let a = audio_armed.clone();
        Callback::from(move |_| { a.set(true); mp.set(MobilePanel::Orion);  }) };
    ...
    html! {
        <div class="app-shell">
            <section class="hero">
                <div class="hero-top">
                    <div class="title-block">
                        ...
                        <p>{"One shared mission timeline. Two instrumentation philosophies. Apollo-style telemetry on the left, Orion-style on the right."}</p>
                        <div class="badge-row">
                            <span class="badge">{"Rust + Yew + WASM"}</span>
                            <span class="badge">{"Same-stage dual instrumentation"}</span>
                            <span class="badge">{"Mini charts + trajectory view"}</span>
                            <span class="badge">{"External WAV cues"}</span>
                    ...
                    <div class="controls">
                        ...
                        <div class="control-group">
                            <label>{"Speed"}</label>
                            <button type="button"
                                class={classes!("btn", if (*speed - 1.0).abs()  < f64::EPSILON { "active" } else { "" })}
                                onclick={on_speed_1}>{"1×"}</button>
                            <button type="button"
                                class={classes!("btn", if (*speed - 5.0).abs()  < f64::EPSILON { "active" } else { "" })}
                                onclick={on_speed_5}>{"5×"}</button>
                            <button type="button"
                                class={classes!("btn", if (*speed - 15.0).abs() < f64::EPSILON { "active" } else { "" })}
                                onclick={on_speed_15}>{"15×"}</button>
                        ...
                        <div class="control-group" style="flex-direction:column; align-items:flex-start; min-width:260px;">
                            <label style="margin-bottom:6px;">{"Audio"}</label>
                            <div class="badge">
                                { format!("{} — {}",
                                    if *audio_armed { "Armed (user gesture received)" } else { "Pending first click" },
                                    (*audio_status).clone()) }
                            </div>
                        </div>
                    </div>
                            ...
                            </div>
                            {
                                match *mobile_panel {
                                    ...
                                    MobilePanel::Center => html! {
                                        <CenterColumn state={state.clone()} total_progress_pct={total_progress_pct}
                                            path_progress={path_progress} logs={(*logs).clone()} />
                                    },
            ...
            <div class="footer-line">
                {"V5.0: External WAV files (assets/audio/) replace embedded base64. HtmlAudioElement plays cues on user-gesture-armed reentry events. AudioContext dependency removed."}
            </div>
        </div>
    }
}
            ...
            <div class="panel-content">
                    ...
                    if props.state.phase == MissionPhase::Reentry {
                        html! {
                            <div class="card apollo-card">
                                ...
                                <div class="big-value apollo-value" style="font-size:1.35rem;">
                                    {if reentry_blackout_active(&props.state) { "COMM BLACKOUT" } else { parachute_stage(&props.state) }}
                                </div>
                                <div class="subline" style="margin-top:8px;">
                                    {if reentry_blackout_active(&props.state)
                                        { "Telemetry expected to drop during peak heating." }
                                        else { "Parachute and recovery timeline driving terminal events." }}
                                </div>
                            </div>
                        }
                    } else { html! {} }
                ...
                <div class="card apollo-card">
                    ...
                    <div class="signal-lamps">
                        <span class="lamp">
                            <span class={classes!("lamp-dot", if d.lamp_guid { "on" } else { "" })}></span>{"GUID"}
                        </span>
                        <span class="lamp">
                            <span class={classes!("lamp-dot", if d.lamp_comm { "on" } else { "" })}></span>{"COMM"}
                        </span>
                        <span class="lamp">
                            <span class={classes!("lamp-dot", if d.lamp_prop { "on" } else { "" })}></span>{"PROP"}
                        </span>
                    </div>
                </div>
                ...
                <div class="card apollo-card">
                    <h3>{"Flight Trend"}</h3>
                    <Sparkline values={props.history.altitude.clone()} stroke_class="#d9d66c" label="ALT trend" />
                    <Sparkline values={props.history.velocity.clone()} stroke_class="#d9d66c" label="VEL trend" />
                </div>
            </div>
        </div>
    }
        ...
        <div class="panel-shell orion-shell">
            ...
            <div class="panel-content">
                            ...
                            <div class="card orion-card">
                                ...
                                <div class="orion-status-row">
                                    <span class="status-chip">
                                        {if reentry_blackout_active(&props.state) { "Telemetry Blackout" } else { "Telemetry Restored" }}
                                    </span>
                                    <span class="status-chip">{parachute_stage(&props.state)}</span>
                                </div>
                                <div class="subline" style="margin-top:10px;">
                                    {orion_log_line(props.state.phase, props.state.phase_progress)}
                                </div>
                            </div>
                        }
                    } else { html! {} }
                ...
                <div class="card orion-card">
                    ...
                    <div class="overlay-readout">
                        ...
                        <div class="overlay-metrics">
                            {format!(
                                "Distance to Earth: {}. Distance to the Moon: {}. Velocity: {}. Mission Elapsed Time: {}",
                ...
                <div class="card orion-card">
                    ...
                    <Sparkline values={props.history.altitude.clone()} stroke_class="#73d7ff" label="Altitude trend" />
                    <Sparkline values={props.history.velocity.clone()} stroke_class="#73d7ff" label="Velocity trend" />
                    <Sparkline values={props.history.comm.clone()}     stroke_class="#73d7ff" label="Comm link trend" />
                </div>
            </div>
        </div>
    }
    ...
    html! {
        <div class="center-column">
            <div class="phase-card">
                ...
                <div class="progress-wrap">
                    <div class="progress-bar">
                        <div class="progress-fill orion-fill"
                             style={format!("width:{:.1}%;", props.total_progress_pct)}></div>
                    </div>
                </div>
            </div>
            ...
            <div class="center-card">
                ...
                <div class="path-box" style="margin-top:10px;">
                    <svg viewBox="0 0 220 260" aria-label="trajectory diagram">
                        ...
                        <circle cx="36"  cy="210" r="18"  fill="url(#earthGlow)" />
                        <circle cx="184" cy="60"  r="12"  fill="url(#moonGlow)"  />
                        <path d="M 36 210 Q 85 55 184 60"
                              fill="none" stroke="rgba(125,242,255,0.55)"
                              stroke-width="3" stroke-dasharray="5 5" />
                        <path d="M 184 60 Q 148 170 36 218"
                              fill="none" stroke="rgba(255,255,255,0.26)"
                              stroke-width="2.5" stroke-dasharray="4 4" />
                        <circle cx={format!("{:.2}", current_x)}
                                cy={format!("{:.2}", current_y)}
                                r="5.8" fill="rgba(125,242,255,0.95)" />
            ...
            <div class="bottom-note">
                {"One shared simulated mission state transformed into two distinct display languages. Apollo remains terse and procedural. Orion remains modern, graphic-rich, and broadcast-friendly."}
            </div>
        </div>
    }
}
